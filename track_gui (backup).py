...
###########################################################################################
    def catchUseJoystickEvent(self, state):
        ...
        if CheckState == True:
            ...
            joyThread = threading.Thread
            joy = Joystick_Thread(73, joyThread, 250)
...
###########################################################################################
	...
	self.progressSpText.setFixedWidth(80)
        ...
        joyThread = threading.Thread
        joy = Joystick_Thread(74, joyThread, 250)

        #joyElDir = 0
        #joyAzDir = 0
        #joySpeed = 0

        #self.progressUD.setValue(joyElDir)
        #self.progressLR.setValue(joyAzDir)
        #self.progressSpeed.setValue(joySpeed)
